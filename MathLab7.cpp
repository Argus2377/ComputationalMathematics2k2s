...
double function(double x, double p[4]) {
...
double bisection(double a, double b, double(&p)[4]) {
    ...
    while (true) {
        ...
        if (function(a, p) * function(c, p) < 0)
        { // определяем границы, где находится корень
            b = c;
        }
        else
        {
...
void calculationEigenvalues(double(&p)[4], double(&a)[4]) { // Считаем собственные числа матрицы
    ...
    cout << "Sobstvenniye chisla:" << endl;
    for (int i = 0; i < 4; i++) {
        cout << "lmbd" << i << ": " << a[i] << endl;
    }
        cout << endl;
}
    ...
    void calculationEigenvectors(double (&Y)[4][5], double (&p)[4], double (&a)[4]) {
        cout << "Sobstvenniye vektora:"<<endl;
        double q[5]{0.0};
        double X[4]{0.0};
        double temp[4]{0.0};
        for (int i = 0; i < 4; i++) {
